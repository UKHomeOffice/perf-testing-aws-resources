...
policy_name = "#{bucket_name}-bucket-delete"
role_name = policy_name

control 'bucket-delete-policy-least-privilege' do
  impact 0.7                                # The criticality, if this control fails.
  title 'Check delete policy privileges'             # A human-readable title
  desc 'Check that the delete policy associated with the bucket applies the principle of least privilege'
  ...
  describe aws_iam_policy(policy_name) do
    ...
    # should allow to s3:DeleteObject* on the bueckt's objects
    it { should have_statement(
      Effect: 'Allow',
    ...
    # should not allow s3:DeleteBucket* actions on the bucket or objects
    it { should_not have_statement(
      Effect: 'Allow',
